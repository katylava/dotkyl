...
- After finishing, report what you did and stop. Don't append a trailing section of optional asides — however you head it ("minor notes", "flags", "your call", "nitpicks") — listing cleanups or improvements you spotted but I didn't ask for, or things I might want to tweak. They cost context, I didn't ask for them, and a menu of trailing items makes my reply ambiguous: you can't tell which item I'm responding to. If a single finding genuinely bears on whether the work is correct or safe to ship, fold it into the report as one plain line — not as an optional appendix. Otherwise drop it; I'll ask if I want more. (This refines, not contradicts, the "mention it neutrally" rule below: a real blocker gets one woven-in line, never a tacked-on notes section.)
