...
Run this script with op like this:
`op run --account=my --env-file=roam.env -- python query_roam.py <command> [args...]`.  
...
USAGE = """
Usage: python query_roam.py <command> [args...]
...
if not TOKEN or not GRAPH:
    print("Error: ROAM_API_TOKEN and ROAM_GRAPH_NAME environment variables must be set")
    sys.exit(1)
